...
This guide explains how waygate behaves when your application runs across multiple instances: load-balanced replicas, rolling deploys, or horizontally-scaled containers. It covers what each backend guarantees, where the boundaries are, and how to choose the right setup for your architecture.

---

## The core question: where does state live?

Every request that hits waygate goes through a single chokepoint: `engine.check()`. The engine reads route state from the backend, evaluates it against the request, and either allows it through or raises a `WaygateException`. In a single-instance deployment this is straightforward. In a multi-instance deployment the question becomes: **when Instance A changes the state of a route, how quickly does Instance B see that change?**
...
State lives in a Python `dict` inside the `WaygateEngine` object. It is never written to disk, never shared over a network, and is lost the moment the process exits.

**In a multi-instance deployment, each instance has its own isolated world.** A `waygate disable` command that reaches Instance A through the admin API will disable the route on Instance A only. Instance B continues serving the route as if nothing happened.
...
graph LR
    ...
    E -->|reads/writes| F["waygate-state.json"]
    CLI["waygate CLI"] -->|REST API| E
    CLI -.->|reads same file| F
```

The CLI talks to the app's `WaygateAdmin` REST API. The API writes to the in-memory cache (which flushes to disk). The file persists across restarts. This is the complete, correct workflow for `FileBackend`. No cross-instance sync is needed because there is only one instance.
...
waygate:route-index           SET      All registered route paths (safe SMEMBERS instead of KEYS scan)
waygate:audit                 LIST     Global audit log, newest-first (LPUSH + LTRIM 1000)
waygate:audit:path:{path}     LIST     Per-path audit log for O(limit) filtered queries
waygate:rlpolicy:{METHOD:path} STRING  JSON-serialised RateLimitPolicy
waygate:rl-policy-index       SET      All registered rate limit policy keys
waygate:global_invalidate     CHANNEL  Pub/sub — fires on every global maintenance config change
waygate:rl-policy-change      CHANNEL  Pub/sub — fires on every rate limit policy set or delete
waygate:changes               CHANNEL  Pub/sub — fires on every set_state call
```

`waygate:route-index` exists specifically to avoid `KEYS waygate:state:*`. `KEYS` is an O(keyspace) blocking command that freezes Redis on busy production instances. `SMEMBERS` on the dedicated route-index set is safe to use at any time.
...
```python
pipe.set(f"waygate:state:{path}", payload)   # write state
pipe.sadd("waygate:route-index", path)       # maintain index
pipe.publish("waygate:changes", payload)     # notify subscribers
await pipe.execute()
```

The write and the notification are atomic. Any instance subscribed to `waygate:changes` (the dashboard SSE endpoint) receives the new `RouteState` immediately. The next `engine.check()` call on any instance reads fresh state from Redis, as there is no in-process per-route state cache.
...
sequenceDiagram
    ...
    A->>R: SET waygate:global__ sentinel
    A->>R: PUBLISH waygate:global_invalidate "1"
...
`WaygateEngine.start()` creates a background `asyncio.Task` that runs `_run_global_config_listener()`. This task iterates `backend.subscribe_global_config()` indefinitely. Each message arrival from `waygate:global_invalidate` triggers an immediate cache flush. The next `engine.check()` call re-fetches `GlobalMaintenanceConfig` from Redis, observing the change made by Instance A.
...
- `WaygateEngine.__aenter__`: for CLI scripts using `async with WaygateEngine(...) as engine:`
- `WaygateRouter.register_waygate_routes()`: at FastAPI application startup
...
sequenceDiagram
    ...
    A->>R: SET waygate:rlpolicy:{key}
    A->>R: PUBLISH waygate:rl-policy-change
    R-->>B: _run_rl_policy_listener()
    B->>B: _rate_limit_policies[key] = new_policy
```

`WaygateEngine.start()` creates a `waygate-rl-policy-listener` background task. Each message on `waygate:rl-policy-change` carries the full policy payload (for `set`) or just the key (for `delete`), so the receiving instance applies the delta directly without an extra Redis round-trip.
...
`MemoryBackend` now implements `subscribe_rate_limit_policy()` via an `asyncio.Queue`, meaning the Waygate Server can broadcast RL policy events to all SDK clients connected over SSE — see [Rate limit policy SSE propagation](#rate-limit-policy-sse-propagation) below.
...
sequenceDiagram
    ...
    A->>R: SET waygate:state:GET:/api/payments
    A->>R: SADD waygate:route-index
    A->>R: PUBLISH waygate:changes
    R-->>B: dashboard SSE subscriber notified

    C->>B: POST /api/payments
    B->>R: GET waygate:state:GET:/api/payments
...
Without deduplication, webhooks would fire once per instance per event: three instances would mean three `maintenance_on` deliveries for a single window activation. waygate prevents this with a `SET NX` claim in Redis before any HTTP POST is sent.
...
sequenceDiagram
    ...
    A->>R: SET waygate:webhook:dedup:{key} NX EX 60
    R-->>A: OK (claimed)
    A->>A: POST webhook fires ✅

    B->>B: dedup_key = sha256(event + state)
    B->>R: SET waygate:webhook:dedup:{key} NX EX 60
    R-->>B: nil (already claimed)
    B->>B: skip ❌

    C->>C: dedup_key = sha256(event + state)
    C->>R: SET waygate:webhook:dedup:{key} NX EX 60
...
The OpenAPI schema filter (`apply_waygate_to_openapi`) uses a monotonic `_schema_version` counter inside each `WaygateEngine` instance to decide when to rebuild the cached schema. This counter is incremented on every local state change and is not shared across instances.
...
If you need fully consistent OpenAPI schemas across instances, you can force a cache refresh by calling `app.openapi.cache_clear()` (or the equivalent for your schema caching setup) in response to `waygate:changes` pub/sub messages.

---

## Rate limit policy SSE propagation

When using **Waygate Server + WaygateSDK**, rate limit policies set via the CLI (`waygate rl set`) or the dashboard are immediately broadcast to all connected SDK clients over the persistent SSE stream — no restart required.
...
When `WaygateServerBackend` receives an `rl_policy` event, it updates its local `_rl_policy_cache` and notifies the engine's existing `_run_rl_policy_listener()` background task, which applies the policy change to `engine._rate_limit_policies` immediately. The propagation path is:

```mermaid
flowchart TD
    CLI["waygate rl set GET:/api/pay 100/min"]
    SS["WaygateServer\nengine.set_rate_limit_policy()"]
    MB["MemoryBackend\nasyncio.Queue fan-out"]
    SSE["sdk_events SSE stream"]
    BG["WaygateServerBackend\n_listen_sse()"]
...
Regardless of backend, waygate will **never take down your API due to its own backend being unreachable**. Every `backend.get_state()` call is wrapped in a try/except inside `engine.check()`:
...
try:
    ...
    logger.exception("waygate: backend error — failing open")
    return None    # allow the request through
```

If Redis goes down, all `engine.check()` calls fail-open: every request passes through as if all routes are `ACTIVE`. Waygate logs the errors at `ERROR` level but never surfaces them as HTTP failures to your clients. Your API continues to serve traffic; you just lose the protection layer temporarily.
...
- [ ] Set `WAYGATE_REDIS_URL` via environment variable; do not hardcode credentials
- [ ] Use Redis database `0` for application data, a separate DB (e.g. `15`) for tests
- [ ] Mount `WaygateAdmin` behind authentication (`auth=("user", "pass")` or a custom `WaygateAuthBackend`)
...
- [ ] Set `WAYGATE_ENV` correctly per deployment environment so `@env_only` gating works as intended
