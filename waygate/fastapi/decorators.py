"""FastAPI route decorators for waygate.

Every decorator returns a :class:`_WaygateCallable` — a dual-purpose object:

* **As a decorator**: stamps ``__waygate_meta__`` on the route function.
  ``WaygateRouter`` reads this at startup and registers the initial state in
...
* **As a FastAPI dependency** (via ``Depends()``): enforces the route state
  ...
  1. **Engine from app state** (recommended — zero config per route):
     Call ``configure_waygate(app, engine)`` once (or use ``WaygateMiddleware``
     which does this automatically).  All decorator deps then find the engine
     via ``request.app.state.waygate_engine`` without any ``engine=`` argument.

  2. **Explicit engine** (``engine=engine``): overrides the app-state lookup.
     Useful when you have multiple engines or don't use ``configure_waygate``.
...
All three styles use the same import — no ``_dep`` suffix, no separate class::
    ...
    from waygate.fastapi import maintenance, disabled, env_only, configure_waygate

    # Once, in app setup:
    configure_waygate(app, engine)  # or: app.add_middleware(WaygateMiddleware, engine=engine)
...
registered in the backend (either via ``WaygateRouter`` at startup, or by
...
# WaygateException that triggered the block, and returns a Starlette Response.
...
_WAYGATE_ENGINE_ATTR = "waygate_engine"
...
def _resolve_engine(explicit: Any, request: Request) -> Any:
    ...
    2. ``request.app.state.waygate_engine`` — set by ``configure_waygate`` or
       ``WaygateMiddleware`` at ASGI lifespan startup.
    3. ``None`` — no engine available; caller falls back to inline logic.
    """
    if explicit is not None:
        return explicit
    return getattr(request.app.state, _WAYGATE_ENGINE_ATTR, None)
...
def _engine_dep_raise(engine: Any, request: Request) -> None:
    ...
    within such a thread.  WaygateExceptions are caught here and re-raised
    as ``HTTPException``.
    """
...
def _stamp(func: F, meta: dict[str, Any]) -> F:
    """Attach *meta* as ``__waygate_meta__`` on *func*, merging if already set."""
    existing: dict[str, Any] = getattr(func, "__waygate_meta__", {})
    existing.update(meta)
    func.__waygate_meta__ = existing  # type: ignore[attr-defined]
    return func
...
class _WaygateCallable:
    """Dual-purpose callable: decorator or FastAPI dependency.

    When called with a **function** (decorator use), stamps ``__waygate_meta__``
...
def maintenance(
    ...
    engine:
        ...
        ``app.state.waygate_engine`` (set by ``configure_waygate`` or
        ``WaygateMiddleware``).  Pass explicitly only when you need to target a
    ...
    Zero-config dep (engine resolved from app state automatically)::
        ...
        configure_waygate(app, engine)  # once
    ...
    return _WaygateCallable(meta=meta, dep_raise=dep_raise)
...
def env_only(
    *envs: str, engine: Any = None, response: ResponseFactory | None = None
    ...
    engine:
        Optional explicit engine override.  When omitted, the dep looks up
        ``app.state.waygate_engine``.  If no engine is found anywhere, the dep
        fails open (use ``WaygateMiddleware`` for middleware-level enforcement).
    response:
        Optional custom response factory — a sync or async callable with
        signature ``(request: Request, exc: Exception) -> Response``.
...
def disabled(
    ...
    engine:
        Optional explicit engine override.  When omitted, the dep looks up
        ``app.state.waygate_engine``.  When an engine is available the route
        can be re-enabled at runtime via ``waygate enable <path>`` or the
        dashboard.
    response:
        Optional custom response factory — a sync or async callable with
        signature ``(request: Request, exc: Exception) -> Response``.
    ...
    Examples
    --------
    Zero-config dep (re-enable with ``waygate enable /old-endpoint``)::
...
def deprecated(
    ...
    - ``WaygateRouter`` registers the route with status ``DEPRECATED``.
...
def rate_limit(
    ...
    *,
    ...
    Works as both a decorator (stamps ``__waygate_meta__``) and a FastAPI
    dependency (enforces the limit at request time when the engine is
    available via ``configure_waygate``).
...
def force_active(func: F) -> F:
    """Force a route to bypass all waygate checks.
    ...
    ``Depends()`` dependency.  Waygate checks are enforced by the middleware,
