"""WaygateRouter — drop-in replacement for FastAPI's APIRouter.

At application startup it scans every route for ``__waygate_meta__``
...
directly on the ``FastAPI`` app to benefit from waygate decorators too.
"""
...
async def _register_rate_limit_from_meta(
    path: str, method: str, meta: dict[str, Any], engine: WaygateEngine
) -> None:
    """Register a rate limit policy from ``__waygate_meta__`` if present.
    ...
    try:
        ...
        logging.getLogger(__name__).warning(
            "waygate: failed to register rate limit for %s %s: %s", method, path, exc
        )


async def scan_routes(app: Any, engine: WaygateEngine) -> None:
    ...
    **All routes are registered**, not just decorated ones:
    ...
    - Routes with ``__waygate_meta__`` are registered with their decorator state.
    - Routes without ``__waygate_meta__`` are registered as ``ACTIVE``.
    ...
    ``WaygateRouter`` startup hook or a previous ``scan_routes()`` call) are
    ...
    app:
        ...
        Inside ``WaygateMiddleware`` this is ``request.app``.
    engine:
...
class WaygateRouter(APIRouter):
    """Drop-in replacement for ``fastapi.APIRouter``.

    Identical to ``APIRouter`` in every way except that it registers
    waygate metadata with the engine at startup.

    Parameters
    ----------
    engine:
        The ``WaygateEngine`` instance shared with the middleware.
    **kwargs:
        All other keyword arguments are forwarded to ``APIRouter``.
    """

    def __init__(self, engine: WaygateEngine, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self._waygate_engine = engine
        # Collect (path, meta) pairs discovered via add_api_route.
        self._waygate_routes: list[tuple[str, dict[str, Any]]] = []
        # Collect (path, method, meta) triples for rate limit registration.
        self._waygate_rl_routes: list[tuple[str, str, dict[str, Any]]] = []
        # Register the startup hook so FastAPI picks it up automatically.
        self.on_startup.append(self.register_waygate_routes)
    ...
    @classmethod
    def from_engine(cls, engine: WaygateEngine, **kwargs: Any) -> WaygateRouter:
        """Create a ``WaygateRouter`` bound to *engine*."""
        return cls(engine=engine, **kwargs)

    # ------------------------------------------------------------------
    # Override route registration to detect __waygate_meta__
    # ------------------------------------------------------------------
    ...
    def add_api_route(
        ...
        **kwargs: Any,
        ...
        """Register a route and record any waygate metadata on the endpoint.
        ...
        ``WaygateMiddleware`` resolves via template-path lookup.  For example,
        ...
        if methods:
            ...
            for method in sorted(methods):
                self._waygate_routes.append((f"{method}:{full_path}", meta))
                # Collect rate limit registrations for startup.
                if meta.get("rate_limit"):
                    self._waygate_rl_routes.append((full_path, method, meta))
        else:
            # No methods known — fall back to path-level (all-methods) key.
            self._waygate_routes.append((full_path, meta))
            if meta.get("rate_limit"):
                self._waygate_rl_routes.append((full_path, "ALL", meta))

    # ------------------------------------------------------------------
    # Startup: register all discovered routes with the engine
    # ------------------------------------------------------------------

    async def register_waygate_routes(self) -> None:
        """Register all waygate-decorated routes with the engine.

        Call this during application startup (e.g. via a ``lifespan``
        handler or ``on_startup`` event).  ``WaygateRouter`` calls this
        ...
        for path, method, meta in self._waygate_rl_routes:
            await _register_rate_limit_from_meta(path, method, meta, self._waygate_engine)
        await self._waygate_engine.register_batch(list(self._waygate_routes))
        await self._waygate_engine.start()
    ...
    def _get_startup_handler(self) -> Callable[[], Any]:
        """Return an async startup handler that registers waygate routes."""

        async def _startup() -> None:
            await self.register_waygate_routes()
