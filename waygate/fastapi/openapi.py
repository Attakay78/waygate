"""OpenAPI schema filter and docs UI customisation for waygate.

``apply_waygate_to_openapi`` monkey-patches ``app.openapi()`` so that the
...
- ``MAINTENANCE`` — ``x-waygate-status`` extension added; markdown warning
                    prepended to the operation description; summary prefixed
                    with ``🔧 ``.
- **Global maintenance ON** — ``x-waygate-global-maintenance`` extension added
...
``setup_waygate_docs`` replaces both ``/docs`` (Swagger UI) and ``/redoc``
...
from waygate.core.models import GlobalMaintenanceConfig, RouteState, RouteStatus
...
_MAINTENANCE_DESCRIPTION_BANNER = (
    "> 🔧 **Waygate: MAINTENANCE** — {reason}\n>\n"
    "> This endpoint is temporarily unavailable.  "
    "Check back later or contact your API team.\n\n"
)


def apply_waygate_to_openapi(app: FastAPI, engine: WaygateEngine) -> None:
    """Patch ``app.openapi()`` to filter paths based on waygate route state.
        ...
        if global_cfg is not None and global_cfg.enabled:
            ...
            for path in list(filtered.keys()):
                ...
                for key, value in path_item.items():
                    ...
                    if value.get("x-waygate-status") == "maintenance":
                        patched_for_global[key] = value
                        continue
            ...
            # The sticky HTML banner (rendered by setup_waygate_docs) is the
            ...
            schema["info"]["x-waygate-global-maintenance"] = {
                "enabled": True,
            ...
            schema["info"] = dict(schema.get("info", {}))
            schema["info"]["x-waygate-global-maintenance"] = {"enabled": False}
...
def _annotate_maintenance(operation: dict[str, Any], state: RouteState) -> dict[str, Any]:
    ...
    * ``x-waygate-status`` / ``x-waygate-reason`` — machine-readable extensions
      picked up by ``setup_waygate_docs`` to apply visual styling in Swagger UI.
    ...
    * ``summary`` — ``🔧 `` prefix added so the maintenance state is visible
    ...
    patched["x-waygate-status"] = "maintenance"
    patched["x-waygate-reason"] = state.reason or ""
...
#
# Responsibilities:
#  1. Read ``info['x-waygate-global-maintenance']`` from the live OpenAPI spec.
...
#  4. Add per-operation orange badges for routes with x-waygate-status=maintenance.
...
<style>
  ...
  #waygate-global-banner {
    ...
    animation: waygate-pulse 2.5s ease-in-out infinite;
  }
  @keyframes waygate-pulse {
    0%, 100% { box-shadow: 0 4px 16px rgba(198,40,40,0.5); }
    50%       { box-shadow: 0 4px 32px rgba(198,40,40,0.9); }
  }
  #waygate-global-banner .waygate-banner-title {
    font-size: 22px;
    font-weight: 900;
    letter-spacing: 2px;
  ...
  #waygate-global-banner .waygate-banner-reason {
    font-size: 15px;
    opacity: 0.92;
    margin-bottom: 4px;
  }
  #waygate-global-banner .waygate-banner-exempt {
    font-size: 12px;
    opacity: 0.75;
    font-style: italic;
  }
  ...
  #waygate-ok-chip .dot {
    ...
    border-radius: 50%;
    background: #69f0ae;
    animation: waygate-blink 2s ease-in-out infinite;
  }
  @keyframes waygate-blink {
    0%, 100% { opacity: 1; }
    50%       { opacity: 0.3; }
  }

  /* --- Per-operation maintenance styling --- */
  .waygate-maintenance-block {
    border-left: 4px solid #ff9800 !important;
    background: rgba(255, 152, 0, 0.07) !important;
  }
  .waygate-maintenance-badge {
...
(function () {
  ...
  function applyGlobalBanner(spec) {
    var gm = (spec.info || {})['x-waygate-global-maintenance'] || {};
    var bannerEl = document.getElementById('waygate-global-banner');
    var chipEl   = document.getElementById('waygate-ok-chip');
    ...
    if (gm.enabled) {
      ...
      if (!bannerEl) {
        ...
        bannerEl.id = 'waygate-global-banner';
      ...
      var exemptHtml = exempts.length
        ? '<div class="waygate-banner-exempt">Exempt: ' +
          exempts.join(' &bull; ') + '</div>'
        : '';

      bannerEl.innerHTML =
        '<div class="waygate-banner-title">🔴 Site-Wide Maintenance In Progress</div>' +
        '<div class="waygate-banner-reason">' + escHtml(reason) + '</div>' +
        exemptHtml;
  ...
  function buildMaintenanceMap(spec) {
    ...
    Object.keys(paths).forEach(function (path) {
      ...
      ['get','post','put','patch','delete','head','options'].forEach(function (m) {
        var op = methods[m];
        if (op && op['x-waygate-status'] === 'maintenance') {
          map[m.toUpperCase() + ':' + path] = op['x-waygate-reason'] || '';
        }
      });
    });
    return map;
  ...
  function applyBadges(maintenanceMap) {
    document.querySelectorAll('.opblock').forEach(function (el) {
      ...
      if (el.classList.contains('waygate-maintenance-block')) return;

      el.classList.add('waygate-maintenance-block');

      var summary = el.querySelector('.opblock-summary');
      if (summary && !summary.querySelector('.waygate-maintenance-badge')) {
        var badge = document.createElement('span');
        badge.className   = 'waygate-maintenance-badge';
...
def setup_waygate_docs(app: FastAPI, engine: WaygateEngine) -> None:
    """Replace the default ``/docs`` and ``/redoc`` with waygate-aware versions.
    ...
    * **Per-route maintenance** — orange left-border + ``🔧 MAINTENANCE``
    ...
    (CLI ``waygate global enable/disable``) without a page reload.

    Call *after* ``apply_waygate_to_openapi``::
    ...
    def _inject(base_html: str) -> str:
        """Embed openapi URL + waygate script into an HTML page."""
        base_html = base_html.replace(
            "<body>",
            f'<body data-openapi-url="{openapi_url}">',
            1,
        )
        return base_html.replace("</body>", f"{_WAYGATE_DOCS_SCRIPT}\n</body>", 1)

    @app.get(docs_url, include_in_schema=False)
    async def waygate_swagger_ui() -> HTMLResponse:
    ...
    async def waygate_redoc() -> HTMLResponse:
...
def _fetch_states(engine: WaygateEngine) -> list[RouteState] | None:
...
def _fetch_global_config(engine: WaygateEngine) -> GlobalMaintenanceConfig | None:
