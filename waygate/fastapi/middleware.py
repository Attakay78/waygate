"""WaygateMiddleware — ASGI middleware that enforces route lifecycle policies.

Every inbound request passes through ``dispatch()``.  The middleware:

1. Scans all app routes for ``__waygate_meta__`` at ASGI lifespan startup so
...
4. Translates ``WaygateException`` subclasses into structured JSON responses.
5. Injects RFC headers for deprecated routes and ``Retry-After`` for
   maintenance responses.

Route scanning — how and when it happens
-----------------------------------------
``WaygateMiddleware`` hooks into the ASGI lifespan to scan *all* app routes for
``__waygate_meta__`` immediately after the server's startup events complete
(which means ``WaygateRouter.on_startup`` hooks have already run).
...
class WaygateMiddleware(BaseHTTPMiddleware):
    ...
    responses:
        Optional mapping of waygate status → response factory used as the
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        event handlers (including ``WaygateRouter.on_startup``) have already
            ...
            if app is not None:
                ...
                async def _send(message: Any) -> None:
                    if message.get("type") == "lifespan.startup.complete":
                        ...
                        # after all startup events (WaygateRouter hooks, etc.)
                        # have already run.
                        await self._do_scan(app)
                        self.engine.scheduler.start_polling()
                        # Make the engine discoverable by decorator deps via
                        # request.app.state.waygate_engine — this is what lets
                        # maintenance()/disabled()/env_only() find the engine
                        # without needing engine= on every call.
                        from waygate.fastapi.dependencies import configure_waygate

                        configure_waygate(app, self.engine)
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        """Intercept the request and run waygate checks."""
        ...
        # Always skip internal/docs prefixes for waygate enforcement.
        ...
        if is_force_active:
            # @force_active routes bypass all waygate checks by default.
    ...
    def _resolve_route(self, request: Request) -> tuple[bool, str | None, Any, Any]:
        ...
        - ``is_force_active`` — ``True`` when the matched endpoint carries
          ``@force_active`` and should bypass all waygate checks.
    ...
    async def _call_response_factory(factory: Any, request: Request, exc: Exception) -> Response:
        ...
        The factory receives the live ``Request`` and the ``WaygateException``
