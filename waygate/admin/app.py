"""WaygateAdmin — unified admin ASGI app.

Replaces :func:`~waygate.dashboard.app.WaygateDashboard` as the recommended
entry-point.  A single mount provides:

* **Dashboard UI** — the HTMX / Jinja2 admin panel (same as before)
* **REST API** — JSON endpoints the ``waygate`` CLI uses as its back-end
* **Auth** — token-based session management shared by both surfaces

Usage::

    from waygate.admin import WaygateAdmin
...
* :class:`~waygate.admin.auth.WaygateAuthBackend` subclass instance — custom

Dashboard sessions
------------------
After logging in via ``/login`` the browser receives an ``HttpOnly`` session
cookie (``waygate_session``).  The actor stored in audit entries is the
authenticated username; platform is ``"dashboard"``.

CLI tokens
----------
``POST /api/auth/login`` returns a bearer token the CLI stores in
``~/.waygate/config.json``.  All subsequent CLI requests send
``X-Waygate-Token: <token>``; the actor is the authenticated username
and platform is ``"cli"``.

No-auth mode
------------
When ``auth=None`` every request is accepted.  The actor defaults to
``"anonymous"`` unless the client sends an ``X-Waygate-Actor`` header, in
...
from waygate.admin import api as _api
from waygate.admin.auth import AuthConfig, TokenManager, auth_fingerprint, make_auth_backend
from waygate.core.engine import WaygateEngine
from waygate.dashboard import routes as _dash
...
class _AuthMiddleware(BaseHTTPMiddleware):
    """Inject ``waygate_actor`` and ``waygate_platform`` into ``request.state``.

    * If auth is **not** configured every request passes through; actor is
      taken from the ``X-Waygate-Actor`` header or falls back to
      ``"anonymous"``.
    * If auth **is** configured:

      1. Try ``X-Waygate-Token: <token>`` (CLI / programmatic access).
      2. Try ``waygate_session`` cookie (dashboard browser session).
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        if not self._has_auth:
            # No auth — allow everything; honour optional X-Waygate-Actor hint.
            request.state.waygate_actor = request.headers.get("X-Waygate-Actor", "") or "anonymous"
            request.state.waygate_platform = request.headers.get("X-Waygate-Platform", "cli")
        ...
        if any(path == p or path.endswith(p) for p in _PUBLIC_PATHS) or any(
            ...
            request.state.waygate_actor = "anonymous"
            request.state.waygate_platform = "anonymous"
            return await call_next(request)

        # Try X-Waygate-Token header first (CLI / programmatic access).
        token = self._tm.extract_token(request.headers.get("X-Waygate-Token", ""))
        ...
        if token:
            ...
            if result:
                request.state.waygate_actor = result[0]
                request.state.waygate_platform = result[1]
                return await call_next(request)

        # No valid credentials — decide how to reject.
        is_api = "/api/" in path
        is_cli = request.headers.get("X-Waygate-Platform", "") == "cli"
...
def _flag_dashboard_routes() -> list[Route]:
    """Return the flag + segment dashboard UI routes for mounting in WaygateAdmin."""
    return [
...
def _flag_routes() -> list[Route]:
    """Return the flag + segment API routes for mounting in WaygateAdmin."""
    return [
        # ── Flags CRUD ───────────────────────────────────────────────
...
def WaygateAdmin(
    ...
    """Create the unified Waygate admin ASGI app.
    ...
    ----------
    engine:
        The :class:`~waygate.core.engine.WaygateEngine` to administer.
    auth:
        Credentials config.  ``None`` = open access.  Accepts a
        ``(username, password)`` tuple, a list of such tuples, or a custom
        :class:`~waygate.admin.auth.WaygateAuthBackend` instance.
    token_expiry:
    ...
    def _clean_entry_path(entry: object) -> str:
        ...
        if raw.startswith("__waygate:svc_global:") and raw.endswith("__"):
            name = raw[len("__waygate:svc_global:") : -2]
            return f"[{name} Maintenance]"
        if raw.startswith("__waygate:svc_rl:") and raw.endswith("__"):
            name = raw[len("__waygate:svc_rl:") : -2]
    ...
    try:
        version = importlib.metadata.version("waygate")
    ...
    starlette_app = Starlette(
        routes=[
            ...
            *(_flag_dashboard_modal_routes() if enable_flags else []),
            ...
            # ── SDK endpoints (WaygateServerBackend / WaygateSDK) ──────────
