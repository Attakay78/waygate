"""Auth backends and token management for WaygateAdmin.
...
* ``MyAuthBackend()``             — custom :class:`WaygateAuthBackend` subclass
...
class _SingleUserAuth(WaygateAuthBackend):
...
class _MultiUserAuth(WaygateAuthBackend):
...
def make_auth_backend(auth: AuthConfig) -> WaygateAuthBackend | None:
    """Convert *auth* config into a :class:`WaygateAuthBackend` instance.
    ...
    auth:
        ...
        * :class:`WaygateAuthBackend` instance — custom implementation
    """
    if auth is None:
        return None
    if isinstance(auth, tuple):
...
class TokenManager:
    ...
    COOKIE_NAME = "waygate_session"
    TOKEN_HEADER = "X-Waygate-Token"
    ...
    def extract_token(self, token_header_value: str) -> str | None:
        """Pull raw token from the ``X-Waygate-Token`` header value."""
