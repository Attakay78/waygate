"""REST API route handlers for WaygateAdmin.
...
Auth
----
Requests must carry a valid ``X-Waygate-Token: <token>`` header.
When auth is not configured on the server every request is accepted and
the actor defaults to ``"anonymous"``.

Actor / Platform
----------------
Every mutating handler reads ``request.state.waygate_actor`` and
``request.state.waygate_platform`` (injected by the auth middleware) so
...
from waygate.core.models import AuditEntry, MaintenanceWindow, RouteState
...
def _engine(request: Request) -> WaygateEngine:
    """Return the WaygateEngine from app state."""
    return request.app.state.engine  # type: ignore[no-any-return]


def _actor(request: Request) -> str:
    """Return the authenticated actor name from request state."""
    return getattr(request.state, "waygate_actor", "anonymous")


def _platform(request: Request) -> str:
    """Return the authenticated platform from request state."""
    return getattr(request.state, "waygate_platform", "cli")
...
def _extract_token(request: Request) -> str | None:
    value = request.headers.get("X-Waygate-Token", "").strip()
...
async def delete_rate_limit_policy_api(request: Request) -> JSONResponse:
    ...
    (e.g. ``GET:/api/items``).  Use :func:`waygate.cli.client._encode_path`
...
# ---------------------------------------------------------------------------
# SDK endpoints — used by WaygateServerBackend / WaygateSDK clients
# ---------------------------------------------------------------------------
...
async def sdk_events(request: Request) -> StreamingResponse:
    ...
    SDK clients (``WaygateServerBackend``) connect here to keep their
    ...
    * Rate limit policy change::
    ...
    async def _feed_states() -> None:
        try:
            ...
            logger.exception("waygate: SDK SSE state subscription error")
    ...
    async def _feed_rl_policies() -> None:
        try:
            ...
            logger.exception("waygate: SDK SSE RL policy subscription error")
    ...
    async def _feed_flags() -> None:
        try:
            ...
            logger.exception("waygate: SDK SSE flag subscription error")
...
async def sdk_register(request: Request) -> JSONResponse:
    ...
    logger.debug("waygate: SDK registered %d route(s) from app_id=%s", len(results), app_id)
...
async def list_services(request: Request) -> JSONResponse:
    ...
    registered routes with this Waygate Server.  Routes without a service
...
async def sdk_audit(request: Request) -> JSONResponse:
    ...
    SDK clients forward audit entries here so the Waygate Server maintains
...
# ---------------------------------------------------------------------------
#
# These endpoints are only mounted when WaygateAdmin(enable_flags=True).
    ...
    return JSONResponse(
        {
            "error": (
                ...
                "Call engine.use_openfeature() and set enable_flags=True on WaygateAdmin."
            )
        },
        status_code=501,
    )
...
def _flags_not_installed() -> JSONResponse:
    return JSONResponse(
        {
            "error": (
                "Feature flags require the [flags] extra. Install with: pip install waygate[flags]"
            )
        },
        status_code=501,
    )
...
async def create_flag(request: Request) -> JSONResponse:
    ...
    try:
        from waygate.core.feature_flags.models import FeatureFlag
...
async def evaluate_flag(request: Request) -> JSONResponse:
    ...
    try:
        ...
        from waygate.core.feature_flags.models import EvaluationContext
...
async def create_segment(request: Request) -> JSONResponse:
    ...
    try:
        from waygate.core.feature_flags.models import Segment
