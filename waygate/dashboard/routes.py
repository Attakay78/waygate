"""Waygate dashboard HTTP route handlers."""
...
from waygate.core.models import MaintenanceWindow, RouteState
...
def _actor(request: Request) -> str:
    ...
    return getattr(request.state, "waygate_actor", "dashboard")


def _platform(request: Request) -> str:
    """Return the platform from request state (always 'dashboard' for UI actions)."""
    return getattr(request.state, "waygate_platform", "dashboard")
...
def _render_rl_row(tpl: Jinja2Templates, policy: Any, prefix: str) -> str:
    ...
    return html + "<script>document.getElementById('waygate-modal').close()</script>"
...
async def rl_edit(request: Request) -> HTMLResponse:
    ...
    try:
        ...
        return HTMLResponse(
            html,
            headers={"HX-Retarget": "#waygate-modal", "HX-Reswap": "innerHTML"},
        )
    policy = engine._rate_limit_policies.get(composite)
    if policy is None:
        return HTMLResponse("")
...
async def events(request: Request) -> StreamingResponse:
    ...
    ``waygate:update:{path_slug}``.  HTMX receives the event and replaces
    ...
    async def _generate() -> object:
        try:
            async for state in engine.backend.subscribe():
                ...
                yield f"event: waygate:update:{slug}\ndata: {data_lines}\n\n"
        except NotImplementedError:
            # Backend does not support pub/sub — fall through to keepalive loop.
            pass
        except Exception:
            logger.exception("waygate dashboard: SSE subscription error, falling back to keepalive")
...
async def flag_variations_save(request: Request) -> Response:
    ...
    from waygate.core.feature_flags.models import FlagType, FlagVariation
...
async def segment_rule_add(request: Request) -> Response:
    ...
    from waygate.core.feature_flags.models import Operator, RuleClause, SegmentRule
