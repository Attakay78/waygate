...
from shield.fastapi import ShieldMiddleware, ShieldRouter
...
def _make_app(limit_str: str, key: str = "ip") -> tuple[FastAPI, ShieldEngine]:
...
async def test_rate_limit_policy_is_registered_on_engine():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        # Making a request triggers the ASGI startup event (ShieldRouter.register_shield_routes)
        await c.get("/items")
