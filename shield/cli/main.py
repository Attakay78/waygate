...
async def _require_exists(engine: object, key: str) -> None:
    """Raise a ``typer.Exit`` with an error if *key* has no registered state."""
    exists = await engine.route_exists(key)  # type: ignore[attr-defined]
    if not exists:
        err_console.print(
            f"[red]Error:[/red] No registered state for [bold]{key!r}[/bold].\n"
            "  Run [dim]shield status[/dim] to see registered routes.\n"
            "  Routes are registered when the server starts up via ShieldRouter."
        )
        raise typer.Exit(code=1)
...
def status(
    ...
    async def _run():
        ...
        if route:
            ...
            await _require_exists(engine, key)
...
def global_status() -> None:
    ...
    async def _run():
        ...
        if cfg.enabled:
            ...
            console.print(
                "  Include @force_active: "
                "[red]yes[/red]" if cfg.include_force_active else "[green]no[/green]"
            )
            if cfg.exempt_paths:
...
# Alias for pyproject.toml entry-point: shield = "shield.cli.main:cli"
cli = app
