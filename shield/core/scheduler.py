...
class MaintenanceScheduler:
    ...
    ----------
    engine:
        The ``ShieldEngine`` instance used to activate and deactivate
    ...
    async def restore_from_backend(self, actor: str = "scheduler") -> None:
        ...
        try:
            ...
            logger.exception("shield: failed to restore scheduled windows from backend")
            return
    ...
    def stop_polling(self) -> None:
        ...
        if self._poll_task is not None and not self._poll_task.done():
            ...
            logger.info("shield: scheduler polling stopped")
    ...
    def _on_task_done(self, path: str, task: asyncio.Task[None]) -> None:
        ...
        if not task.cancelled() and task.exception() is not None:
            logger.error(
                "shield: scheduler task for %r raised an exception: %s",
                path,
                task.exception(),
            )
