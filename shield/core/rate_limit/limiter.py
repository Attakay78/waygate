"""ShieldRateLimiter — orchestrates rate limit checks against policies.

The limiter is the single entry point for rate limit enforcement.
``ShieldEngine.check()`` calls ``ShieldRateLimiter.check()`` when a policy
...
from shield.core.rate_limit.keys import (
    KeyExtractor,
    handle_missing_key,
    is_exempt,
...
from shield.core.rate_limit.storage import RateLimitStorage

logger = logging.getLogger(__name__)

# Prefix used for all rate limit storage keys to avoid collisions.
_KEY_PREFIX = "shield:ratelimit"
